...
/**
 * Object3D_one 컴포넌트 - 선형 패턴 3D 오브젝트
...
const Object3D_one = () => {
  ...
  useEffect(() => {
    ...
    // ===== 캔버스 크기 설정 =====
    const canvasWidth = (50 * window.innerWidth) / 100;
    const canvasHeight = (40 * window.innerWidth) / 100;
    ...
    scene.add(pivot);
    pivot.position.set(-300, 300, 0);
    pivot.scale.set(2.5, 2.5, 2.5);

    const loader = new SVGLoader();
    loader.load("/objects/object01.svg", (data) => {
    ...
    // ===== 윈도우 리사이즈 이벤트 핸들러 =====
    const handleResize = () => {
      const newCanvasWidth = (50 * window.innerWidth) / 100;
      const newCanvasHeight = (40 * window.innerWidth) / 100;
  ...
  // ===== JSX 렌더링 =====
  return <div ref={mountRef} style={{ width: "50vw", height: "40vw" }} />;
};

export default Object3D_one;