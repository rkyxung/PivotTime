...
const Main = () => {
  ...
  const draw = useCallback(() => {
    ...
    if (mouseX === -1) {
      ...
      if (movementRatio >= 0) {
        ...
        calculatedPaths =
          basePaths + (basePaths - minPaths) * movementRatio;
      }
      targetPaths = Math.max(minPaths, calculatedPaths);
    }

    // 목표 경로 개수를 향해 현재 경로 개수를 부드럽게 보간 (Easing)
    animatedPathsRef.current +=
      (targetPaths - animatedPathsRef.current) * 0.05;
  ...
  useEffect(() => {
     ...
     */
    const handleWheel = (e) => {
      if (e.deltaY < 0 && window.scrollY === 0) { // 스크롤 위로 + 페이지 최상단
        e.preventDefault(); // 기본 스크롤 동작 방지
        setIsZoomed(true); // 줌 아웃 상태로 변경 (3D 오브젝트들 표시)
      } else if (e.deltaY > 0 && isZoomedRef.current) { // 스크롤 아래로 + 줌 아웃 상태
        e.preventDefault(); // 기본 스크롤 동작 방지
  ...
  // ===== JSX 렌더링 부분 =====
  return (
    <main ref={mainRef} className={isZoomed ? "zoom_out" : ""}>
      {/* 네비게이션 바 */}
      <nav>
        <div className="logo">Pivot Time</div> {/* 로고 텍스트 */}
        <ul className="Tab_Switcher"> {/* 탭 스위처 메뉴 */}
          <li className="fever_btn">FEVER</li> {/* FEVER 버튼 */}
          <li className="line"> {/* 구분선 */}
            <svg
        ...
        <div className="toggle&nav"> {/* 토글 및 네비게이션 컨테이너 */}
          <div className="toggle_btn"></div> {/* 토글 버튼 */}
          <ul className="nav"> {/* 네비게이션 메뉴 */}
            ...
            <li className="hamburger_btn"> {/* 햄버거 메뉴 버튼 */}
              <svg
                width="1.51vw" // 너비
                height="1.4vw" // 높이
      ...
      <div className="txtBox fever"> {/* FEVER 텍스트 박스 */}
        <span>GET FEVER,</span> {/* FEVER 텍스트 */}
        <div className="slogan">우리의 전환을 담은 순간</div> {/* FEVER 슬로건 */}
      </div>

      <div className="txtBox pivot"> {/* PIVOT 텍스트 박스 */}
        <div className="slogan">새로운 길을 향해!</div> {/* PIVOT 슬로건 */}
        <span>GO PIVOT!</span> {/* PIVOT 텍스트 */}
      </div>

      <div id="canvas-container" ref={canvasContainerRef}> {/* 2D 캔버스 컨테이너 */}
        <canvas ref={canvasRef} /> {/* 2D 애니메이션 캔버스 */}
      </div>

      <div className="main_title">PIVOT TIME</div> {/* 메인 타이틀 */}

      <div className="info left"> {/* 왼쪽 정보 박스 */}
        Kaywon University of Arts & Design {/* 대학명 */}
        <br /> {/* 줄바꿈 */}
        32nd Delight Insight {/* 졸업전시회명 */}
      </div>
      <div className="info center"> {/* 중앙 정보 박스 */}
        Digital-media.kr {/* 웹사이트 주소 */}
        <br /> {/* 줄바꿈 */}
        degreeshow/2025 {/* 경로 */}
      </div>
      <div className="info right"> {/* 오른쪽 정보 박스 */}
        Kaywon Design Hall 5F {/* 전시장 위치 */}
        <br /> {/* 줄바꿈 */}
        Nov. 22. FRI - Nov. 24. SUN {/* 전시 기간 */}
      </div>

      <div className="sub_objects"> {/* 서브 오브젝트들 컨테이너 (줌 아웃 시 표시) */}
        {/* 개별 3D 오브젝트들 - 각각 독립적인 카메라와 렌더러로 드래그 시 360도 회전 가능 */}
        <div className="objects one"> {/* 선형 패턴 3D 오브젝트 - 왼쪽 위 */}
          <Object3D_one />
        </div>
        <div className="objects two"> {/* 타원형 패턴 3D 오브젝트 - 오른쪽 위 */}
          <Object3D_two />
        </div>
        <div className="objects three"> {/* 곡선 패턴 3D 오브젝트 - 왼쪽 아래 */}
          <Object3D_three />
        </div>
        <div className="objects four"> {/* 다각형 패턴 3D 오브젝트 - 오른쪽 아래 */}
          <Object3D_four />
        </div>
      </div>
    </main>
...
export default Main; // Main 컴포넌트를 기본 내보내기
